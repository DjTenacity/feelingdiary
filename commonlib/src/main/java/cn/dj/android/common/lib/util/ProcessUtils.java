    ...
    public static String getForegroundProcessName() {
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (list.size() > 0) {// 有"有权查看使用权限的应用"选项
                try {
                    ApplicationInfo info = packageManager.getApplicationInfo(App.context.getPackageName(), 0);
                    AppOpsManager aom = (AppOpsManager) App.context.getSystemService(Context.APP_OPS_SERVICE);
                    ...
                    UsageStatsManager usageStatsManager = (UsageStatsManager) App.context.getSystemService(Context.USAGE_STATS_SERVICE);
